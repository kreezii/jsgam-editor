...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"node_modules/@json-editor/json-editor/dist/jsoneditor.js":[function(require,module,exports) {
...
(function (global, factory) {
  ...
  JSONEditor.prototype = {
    ...
    init: function () {
      var self = this;
      this.ready = false;
      this.copyClipboard = null; // full references info
      ...
      this._loadExternalRefs(this.schema, function () {
        self._getDefinitions(self.schema, fetchUrl + '#/definitions/'); // Validator options
        ...
        self.validator = new JSONEditor.Validator(self, null, validator_options); // Create the root editor
    ...
    validate: function (value) {
      if (!this.ready) throw "JSON Editor not ready yet.  Listen for 'ready' event before validating"; // Custom value
    ...
    _validateSchema: function (schema, value, path) {
      ...
      if (typeof value === "number") {
        ...
        if (schema.multipleOf || schema.divisibleBy) {
          ...
          valid = value / divisor === Math.floor(value / divisor); // Use math.js is available
        ...
        if (schema.hasOwnProperty('maximum')) {
          ...
          valid = schema.exclusiveMaximum ? value < schema.maximum : value <= schema.maximum; // Use math.js is available
        ...
        if (schema.hasOwnProperty('minimum')) {
          // Vanilla JS, prone to floating point rounding errors (e.g. .999999999999999 == 1)
          valid = schema.exclusiveMinimum ? value > schema.minimum : value >= schema.minimum; // Use math.js is available
            ...
            if (schema.uniqueItems) {
              ...
              if (schema.dependencies) {
                for (i in schema.dependencies) {
                  if (!schema.dependencies.hasOwnProperty(i)) continue; // Doesn't need to meet the dependency

                  if (typeof value[i] === "undefined") continue; // Property dependency
      ...
      if (schema.links) {
        for (var m = 0; m < schema.links.length; m++) {
          if (schema.links[m].rel.toLowerCase() === "describedby") {
            ...
            var data = this.jsoneditor.root.getValue(); //var template = new UriTemplate(href); //preprocessURI(href));
  ...
  JSONEditor.AbstractEditor = Class.extend({
    ...
    setupWatchListeners: function () {
      ...
      if (this.schema.hasOwnProperty('watch')) {
        ...
        for (var name in this.schema.watch) {
          ...
          if (!root) throw "Could not find ancestor node with id " + first; // Keep track of the root node and path for use when rendering the template
      ...
      if (type === 'image') {
        ...
        this.link_watchers.push(function (vars) {
          ...
          this.link_watchers.push(function (vars) {
            ...
            holder.textContent = data.rel; // When a watched field changes, update the url
    ...
    updateHeaderText: function () {
      if (this.header) {
        var header_text = this.getHeaderText(); // If the header has children, only update the text node's value
        ...
        if (this.header.children.length) {
          for (var i = 0; i < this.header.childNodes.length; i++) {
            ...
            if (window.DOMPurify) this.header.innerHTML = window.DOMPurify.sanitize(header_text);else this.header.textContent = this.cleanText(header_text);
          }
      }
    },
  ...
  JSONEditor.defaults.editors.string = JSONEditor.AbstractEditor.extend({
    ...
    build: function () {
      ...
      if (this.options.expand_height) {
        this.adjust_height = function (el) {
          if (!el) return;
          var i,
              ch = el.offsetHeight; // Input too short
      ...
      this.control = this.theme.getFormControl(this.label, this.input, this.description, this.infoButton); // output element to display the range value when it changes or have default.
    ...
    postBuild: function () {
      this._super(); // Enable cleave.js support if library is loaded and config is available
    ...
    enable: function () {
      if (!this.always_disabled) {
        this.input.disabled = false; // TODO: WYSIWYG and Markdown editors
    ...
    disable: function (always_disabled) {
      ...
      this.input.disabled = true; // TODO: WYSIWYG and Markdown editors
        ...
        if (this.options.wysiwyg && ['html', 'bbcode'].indexOf(this.input_type) >= 0 && window.jQuery && window.jQuery.fn && window.jQuery.fn.sceditor) {
          ...
          self.sceditor_instance.blur(function () {
            ...
            this.SimpleMDE.codemirror.on("change", function () {
              ...
              this.ace_editor.session.getSelection().clearSelection();
              this.ace_editor.resize(); // The theme

              if (JSONEditor.plugins.ace.theme) this.ace_editor.setTheme('ace/theme/' + JSONEditor.plugins.ace.theme); // The mode

              this.ace_editor.getSession().setMode('ace/mode/' + this.schema.format); // Listen for changes
    ...
    onWatchedFieldChange: function () {
      var self = this,
          vars,
          j; // If this editor needs to be rendered by a macro template
  ...
  JSONEditor.defaults.editors.rating = JSONEditor.defaults.editors.integer.extend({
    build: function () {
      var self = this,
          i;
      if (!this.options.compact) this.header = this.label = this.theme.getFormInputLabel(this.getTitle());
      if (this.schema.description) this.description = this.theme.getFormInputDescription(this.schema.description); // Dynamically add the required CSS the first time this editor is used
      ...
      ratingContainer.classList.add('rating-container'); // Contains options for rating
  ...
  JSONEditor.defaults.editors.object = JSONEditor.AbstractEditor.extend({
      ...
      if (this.format === 'grid-strict') {
        ...
        for (i = 0; i < rows.length; i++) {
          ...
          if (isCategoriesFormat) {
            ...
            var newTabs_holder = this.theme.getTopTabHolder(this.schema.title); //child [1] of previous, stores panes
              ...
              if (!isObjOrArray) {
                ...
                if (newTabPanesContainer.childElementCount > 0) {
                  ...
                  if (newTabPanesContainer.firstChild.isObjOrArray) {
                    ...
                    self.theme.insertBasicTopTab(editor.tab, newTabs_holder); //newTabs_holder.firstChild.insertBefore(editor.tab,newTabs_holder.firstChild.firstChild);
                    ...
                    self.theme.addTopTab(newTabs_holder, editor.tab); //Update the basicPane
    ...
    getPropertySchema: function (key) {
      ...
      schema = $extend({}, schema);
      var matched = this.schema.properties[key] ? true : false; // Any matching patternProperties should be merged in
      ...
      if (this.options.table_row) {
        ...
        $each(this.editors, function (key, editor) {
          ...
          throw "Not supported yet";
            ...
            this.title.appendChild(this.addproperty_controls); // Show/Hide button
            ...
            this.editjson_controls.appendChild(this.editjson_holder); // Edit JSON Buttton disabled
    ...
    showEditJSON: function () {
      ...
      this.editjson_holder.style.top = this.editjson_button.offsetTop + this.editjson_button.offsetHeight + "px"; // Start the textarea with the current value
    ...
    refreshAddProperties: function () {
      ...
      var can_add = false,
          ...
          num_props = 0,
          i,
          show_modal = false; // Get number of editors
    ...
    setValue: function (value, initial) {
      var self = this;
      value = value || {};
      if (typeof value !== "object" || Array.isArray(value)) value = {}; // First, set the values for all of the defined properties
  ...
  JSONEditor.defaults.editors.array = JSONEditor.AbstractEditor.extend({
    ...
    addRow: function (value, initial) {
      ...
      var controls_holder = self.rows[i].title_controls || self.rows[i].array_controls; // Buttons to delete row, move row up, and move row down
  ...
  JSONEditor.defaults.editors.table = JSONEditor.defaults.editors.array.extend({
    ...
    addRow: function (value) {
      ...
      var i = this.rows.length;
      self.rows[i] = this.getElementEditor(i);
      var controls_holder = self.rows[i].table_controls; // Buttons to delete row, move row up, and move row down
    ...
    build: function () {
      ...
      $each(this.types, function (i, type) {
        ...
        if (typeof type === "string") {
          ...
          schema = $extend({}, self.schema, type); // If we need to merge `required` arrays
  ...
  JSONEditor.defaults.editors.select = JSONEditor.AbstractEditor.extend({
    setValue: function (value, initial) {
      value = this.typecast(value || ''); // Sanitize value before setting it
      ...
      if (this.select2) {
        if (this.select2v4) this.select2.val(this.input.value).trigger("change");else this.select2.select2('val', this.input.value);
      }
    ...
    setupSelect2: function () {
      ...
      if (window.jQuery && window.jQuery.fn && window.jQuery.fn.select2 && (this.enum_options.length > 2 || this.enum_options.length && this.enumSource)) {
        ...
        this.select2.on('select2-blur', function () {
          if (self.select2v4) self.input.value = self.select2.val();else self.input.value = self.select2.select2('val');
          self.onInputChange();
        });
    ...
    onWatchedFieldChange: function () {
      var self = this,
          vars,
          j; // If this editor uses a dynamic select box
    ...
    enable: function () {
      if (!this.always_disabled) {
        ...
        if (this.select2) {
          if (this.select2v4) this.select2.prop("disabled", false);else this.select2.select2("enable", true);
        }
      }
    ...
    disable: function (always_disabled) {
      ...
      if (this.select2) {
        if (this.select2v4) this.select2.prop("disabled", true);else this.select2.select2("enable", false);
      }
  ...
  JSONEditor.defaults.editors.selectize = JSONEditor.AbstractEditor.extend({
    ...
    onWatchedFieldChange: function () {
      ...
      if (this.enumSource) {
        ...
        for (var i = 0; i < this.enumSource.length; i++) {
          ...
          if (Array.isArray(this.enumSource[i])) {
            ...
            select_titles = select_titles.concat(this.enumSource[i]);
              ...
              var items = vars[this.enumSource[i].source]; // Only use a predefined part of the array
        ...
        var prev_value = this.value; // Check to see if this item is in the list
        ...
        this.enum_options = select_options;
        this.enum_display = select_titles;
        this.enum_values = select_options; // If the previous value is still in the new select options, stick with it
  ...
  JSONEditor.defaults.editors.multiselect = JSONEditor.AbstractEditor.extend({
    ...
    setValue: function (value, initial) {
      var i;
      value = value || [];
      if (typeof value !== "object") value = [value];else if (!Array.isArray(value)) value = []; // Make sure we are dealing with an array of strings so we can check for strict equality
    ...
    setupSelect2: function () {
      if (window.jQuery && window.jQuery.fn && window.jQuery.fn.select2) {
        ...
        var self = this;
        this.select2.on('select2-blur', function () {
          if (self.select2v4) self.value = self.select2.val();else self.value = self.select2.select2('val');
          self.onChange(true);
        });
        this.select2.on('change', function () {
          if (self.select2v4) self.value = self.select2.val();else self.value = self.select2.select2('val');
    ...
    updateValue: function (value) {
      ...
      if (this.select2) {
        if (this.select2v4) this.select2.val(this.value).trigger("change");else this.select2.select2('val', this.value);
      }
  ...
  JSONEditor.defaults.editors.base64 = JSONEditor.AbstractEditor.extend({
    ...
    setFileReaderListener: function (fr_multiple) {
      ...
      fr_multiple.addEventListener("load", function (event) {
        if (self.count == self.current_item_index) {
          ...
          var temp_object = {}; // Create empty object
    ...
    build: function () {
      ...
      if (!this.schema.readOnly && !this.schema.readonly) {
        if (!window.FileReader) throw "FileReader required for base64 editor"; // File uploader
  ...
  JSONEditor.defaults.editors.upload = JSONEditor.AbstractEditor.extend({
    ...
    build: function () {
      var self = this;
      this.title = this.header = this.label = this.theme.getFormInputLabel(this.getTitle()); // Input that holds the base64 string

      this.input = this.theme.getFormInputField('hidden');
      this.container.appendChild(this.input); // Don't show uploader if this is readonly

      if (!this.schema.readOnly && !this.schema.readonly) {
        if (!this.jsoneditor.options.upload) throw "Upload handler required for upload editor"; // File uploader
  ...
  JSONEditor.defaults.editors.arraySelectize = JSONEditor.AbstractEditor.extend({
    ...
    build: function () {
      ...
      this.container.appendChild(this.error_holder); //apply global options to array selectize
    ...
    empty: function (hard) {},
    setValue: function (value, initial) {
      var self = this; // Update the array's value, adding/removing rows when necessary
  ...
  JSONEditor.defaults.editors.starrating = JSONEditor.defaults.editors.string.extend({
    build: function () {
      ...
      for (var i = this.enum_values.length - 1; i > -1; i--) {
        var id = this.key + '-' + i; // form radio elements
  ...
  */
  ...
  JSONEditor.defaults.editors.datetime = JSONEditor.defaults.editors.string.extend({
    build: function () {
      ...
      if (window.flatpickr && typeof this.options.flatpickr == 'object') {
        ...
        this.options.flatpickr.noCalendar = this.schema.format == 'time' ? true : false; // Curently only string can contain range or multiple values
  ...
  JSONEditor.defaults.editors.radio = JSONEditor.defaults.editors.string.extend({
    build: function () {
      ...
      for (var i = 0; i < this.enum_values.length; i++) {
        ...
        var radioLabel = document.createElement('label');
        radioLabel.htmlFor = id;
        radioLabel.classList.add('radio'); // contains the displayed text to the label

        var radioLabelText = document.createElement('span');
        radioLabelText.innerText = this.enum_values[i];
        radioLabelText.classList.add('radio__label'); // permits the addition of styles for the radio itself (if you want it to look differently than browser default)
  ...
  JSONEditor.defaults.editors.describedBy = JSONEditor.AbstractEditor.extend({
    ...
    buildChildEditor: function (ref) {
      ...
      var editor = this.jsoneditor.createEditor(this.jsoneditor.getEditorClass(schema), {
        jsoneditor: this.jsoneditor,
        schema: schema,
        container: holder,
        path: this.path,
  ...
  JSONEditor.defaults.themes.bootstrap3 = JSONEditor.AbstractTheme.extend({
    getSelectInput: function (options) {
      ...
      el.classList.add('form-control'); //el.style.width = 'auto';
  ...
  JSONEditor.defaults.themes.materialize = JSONEditor.AbstractTheme.extend({
    ...
    */
    getFormControl: function (label, input, description, infoText) {
      var ctrl,
          type = input.type; // Checkboxes get wrapped in p elements.
      ...
      ctrl = this._super(label, input, description, infoText); // Not .input-field for select wrappers.

      if (!type || !type.startsWith('select')) ctrl.classList.add('input-field'); // Color needs special attention.
    ...
    */
    getTabHolder: function () {
      var html = ['<div class="col s2">', '   <ul class="tabs" style="height: auto; margin-top: 0.82rem; -ms-flex-direction: column; -webkit-flex-direction: column; flex-direction: column; display: -webkit-flex; display: flex;">', '   </ul>', '</div>', '<div class="col s10">', '<div>'].join("\n");
      var el = document.createElement('div');
    ...
    */
    addInputError: function (input, text) {
      ...
      var parent = input.parentNode,
          el;
      if (!parent) return; // Remove any previous error.

      this.removeInputError(input); // Append an error message div.
    ...
    */
    removeInputError: function (input) {
      ...
      var parent = input.parentElement,
          els;
      if (!parent) return; // Remove all elements having class .error-text.
  ...
  JSONEditor.defaults.iconlibs.materialicons = JSONEditor.AbstractIconLib.extend({
    ...
    getIcon: function (key) {
      ...
      if (!mapping) return null; // @see http://materializecss.com/icons.html
  ...
  (function () {
    if (window.jQuery || window.Zepto) {
      ...
      $.fn.jsoneditor = function (options) {
        ...
        if (options === 'value') {
          if (!editor) throw "Must initialize jsoneditor before getting/setting the value"; // Set value
          ...
          if (!editor) throw "Must initialize jsoneditor before validating"; // Validate a specific value
          ...
          this.data('jsoneditor', editor); // Setup event listeners
...
var _jsonEditor = _interopRequireDefault(require("@json-editor/json-editor"));
...
var jsgamUpload = document.querySelector('#upload'); // Initialize the editor

var editor = new _jsonEditor.default(document.getElementById('editor_holder'), {
...
var eventClickFire = function eventClickFire(el) {
...
var downloadJSONHandler = function downloadJSONHandler() {
...
},{"@json-editor/json-editor":"node_modules/@json-editor/json-editor/dist/jsoneditor.js"}],"../../.npm-packages/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
...
},{}]},{},["../../.npm-packages/lib/node_modules/parcel/src/builtins/hmr-runtime.js","index.js"], null)
//# sourceMappingURL=/jsgam-editor.e31bb0bc.js.map